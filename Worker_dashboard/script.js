...
(function () {
  ...
  // Available balance (same logic as earnings overview)
  const available = p.escrow_balance != null
    ? parseFloat(p.escrow_balance)
    : Math.max(0, allMyJobs
        .filter(j => j.status === 'paid')
        .reduce((s, j) => s + parseFloat(j.amount || 0), 0)
  ...
  // Escrow / pending
  const escrow = allMyJobs
    .filter(j => ['assigned', 'pending_verification', 'verified'].includes(j.status))
    .reduce((s, j) => s + parseFloat(j.amount || 0), 0);

  // Update DOM
  const balEl = document.getElementById('paycard-balance');
  const escEl = document.getElementById('paycard-escrow');
  const nameEl = document.getElementById('paycard-name');

  if (balEl) balEl.textContent = available.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  if (escEl) escEl.textContent = '₦' + escrow.toLocaleString();
  if (nameEl) nameEl.textContent = (user.name || 'ARTISAN').toUpperCase();
}

// Hook into existing render flow
const _origRenderStats = renderStats;
renderStats = function(profile) {
  _origRenderStats(profile);
  syncPaycard();
};

const _origRenderEarnings = renderEarningsOverview;
renderEarningsOverview = function(profile) {
  _origRenderEarnings(profile);
  syncPaycard();
};
